...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    stop_number = config.get(CONF_STOP_NUMBER)
    route_number = config.get(CONF_ROUTE_NUMBER)
...
class MetlinkSensor(Entity):
    ...
    def __init__(self, stop_number, route_number):
        ...
        self._name = 'stop_{stop_number}_route_{route_number}'.format(
            stop_number=stop_number.lower(),
            route_number=route_number.lower()
        )
        self.entity_id = "sensor.metlink_{}".format(self._name)
        self.update()
